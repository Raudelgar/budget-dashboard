const ajax = {
    ...
    processDataReports: function(data){
        let i, d, o = {chart:[], grid: [], pair: []}, s = '', lastDashIndex = 0,
            reportsList = [];
...
function displayTilesContents(o){
    ...
    function renderLoop(i){
        let data, report, reportEl, success = false;

        i  = i || 0;
        //TODO: workking on handle 'NO DATA' scenario on each report;
        if(allReports[i]){
            if(allReports[i].type === 'chart'){
                reportEl = tilesEl.querySelector('tile:nth-child('+ (allReports[i].postRender.index + 1) +') chart');
                report = allReports[i].postRender.chart;
                data = report.params;
                report.init(reportEl, data.width, data.height, data.json, function(response){
                    success = response;
                });

                arrayOfChartReports.push(report);
            } else if(allReports[i].type === 'grid'){
                reportEl = tilesEl.querySelector('tile:nth-child('+ (allReports[i].postRender.index + 1) +') gg');
                data = allReports[i];
                report = data.postRender.grid;
                report.init({containerEl: reportEl, content: data}, function(response){
                    success = response;
                });
            } else {
                success = true;

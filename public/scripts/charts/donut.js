        ...
        init: function(element, width, height, json, callback) {
            ...
            function renderChart(reset) {
                ...
                let percentify = function(startAngle, endAngle){
                    return (Math.round((endAngle - startAngle) / (2 * Math.PI) * 1000) / 10) + '%';
                };
                ...
                function renderDonut() {
                    ...
                    .on('mouseover', function(d, i, el) {
						...
						svg.select('g.legend g.item[arcid="' + arcid + '"]').classed('over', true);
						tooltip.update({data: d.data, meta: metaData});
                	})
                    .on('mouseout', function(d, i, el) {
						let arcid = d3.select(el[i]).attr('arcid');

						svg.select('g.legend g.item[arcid="' + arcid + '"]').classed('over', false);
						tooltip.hide();
					});
                }
                ...
                function renderLegend(reset) {
				    ...
				    let lbls = lgnd
                        ...
                        .on('mouseover', function(d, i, el) {
						    let arcid = d3.select(el[i]).attr('arcid');
						    let thisPath = svg.select('g.pathstack path[arcid="' + arcid + '"]');
				    ...
				    lbls
					    .append('text')
                        .text(function(d, i) {
                            let txt = d.data.label.length > 20 ? d.data.label.slice(0, 17) + '...' : d.data.label;
						    return txt;
					    })
					    .attr('fill', function(d, i){
							return colorPalette(i);
                        ...
                        .attr('size', function(d,i,el) {
						    ...
						    if(!lgnd.largestLabelWidth || lgnd.largestLabelWidth < labelWidth){
				    ...
				    lgnd.attr('transform', function(){
					    ...
					    return('translate(' + Math.round(-legendWidth / 2) + ',' + Math.round(-legendHeight / 2) + ')');
				    });
				...
				function update(e){
					...
					path
                        ...
                        .attrTween('d', function(a, i,el) {
							...
							let interpolateText = d3.interpolate(el[i].current.value, a.value);
							let correspondingLabelEl = svg.select('g.values text:nth-child(' + (i + 1) + ')');
							let correspondingPercentEl = svg.select('g.percents text:nth-child(' + (i + 1) + ')');
							...
							return function(t){
								...
								correspondingPercentEl
									.text('(' + percentify(animPercentComplete.startAngle, animPercentComplete.endAngle) + ')')
									.attr('x', centroid[0])
									.attr('y', centroid[1] + 6)
									.attr('hidden', function(d){
										return !d.value || d.value === null;
